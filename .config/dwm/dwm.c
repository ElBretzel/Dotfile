...
void unmapnotify(XEvent *e) {
  ...
  if ((c = wintoclient(ev->window))) {
    ...
    /* KLUDGE! sometimes icons occasionally unmap
     * their windows, but do _not_ destroy them.
...
void updatesystrayicongeom(Client *i, int w, int h) {
  if (i) {
    ...
    /* force icons into the systray dimensions if
     * they don't want to */
    if (i->h > bh) {
      if (i->w == i->h)
        i->w = bh;
...
void updatesystray(void) {
  ...
  for (w = 0, i = systray->icons; i; i = i->next) {
    /* make sure the background color stays the
     * same */
    wa.background_pixel = scheme[SchemeNorm][ColBg].pixel;
